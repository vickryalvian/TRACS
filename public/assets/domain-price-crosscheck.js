...
function dpcToast(type, title, message, options = {}) {
    if (typeof showToast === 'function') {
        ...
        toast([title, message].filter(Boolean).join(': '), type);
    } else {
        console[type === 'error' ? 'error' : 'log']([title, message].filter(Boolean).join(': '));
    }
}
...
function dpcInitExtensionForm() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        const category = form.querySelector('select[name="tld_category"]');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (assignForm) {
        assignForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    dpcToast('success', 'Task assigned', 'Task assigned successfully.');
                    dpcReloadAfterNotice();
                } else {
                    dpcToast('error', 'Assignment failed', data.message || 'Unknown error');
                }
            })
            .catch(error => {
                console.error('Error:', error);
                dpcToast('error', 'Network error', 'A network error occurred while assigning the task.');
            })
            .finally(() => {
