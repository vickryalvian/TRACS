...
$conn->query("CREATE TABLE IF NOT EXISTS `domain_transfers` (
  ...
  `webnic_reseller_transfer` VARCHAR(100) NULL DEFAULT NULL,
  `notes`                    TEXT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    verify_csrf();
    if (!tracs_user_can($conn, 'domains.manage')) {
    ...
    if ($action === 'create') {
        ...
        $allowed = ['pending','pending transfer','locked','error epp code','move domain','done',
    ...
    if ($action === 'quick_move') {
        ...
        $allowed_move = [''];
        if (tracs_table_exists($conn, 'domain_price_sources')) {
            $allowed_result = $conn->query("SELECT source_name FROM domain_price_sources WHERE source_type = 'registrar' AND is_active = 1");
            if ($allowed_result) {
                while ($allowed_row = $allowed_result->fetch_assoc()) {
                    $allowed_move[] = (string)$allowed_row['source_name'];
                }
            }
        }
        if (count($allowed_move) === 1) { $allowed_move = ['', 'Webnic', 'Resellercamp']; }
...
/* ── Registrar options: shared with Domain Price Crosscheck ──── */
$registrar_options = [];
if (tracs_table_exists($conn, 'domain_price_sources')) {
    $registrar_result = $conn->query("SELECT source_name FROM domain_price_sources WHERE source_type = 'registrar' AND is_active = 1 ORDER BY sort_order ASC, source_name ASC");
    if ($registrar_result) {
        while ($registrar_row = $registrar_result->fetch_assoc()) {
            $registrar_options[] = (string)$registrar_row['source_name'];
        }
    }
}
if (!$registrar_options) {
    $registrar_options = ['Webnic', 'Resellercamp'];
}
      ...
      <div class="dt-inline-group">
        ...
        <select class="form-select dt-inline-input" id="nWebnic">
          ...
          <?php foreach ($registrar_options as $registrar_option): ?>
            <option value="<?= esc($registrar_option) ?>"><?= esc($registrar_option) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
    ...
    <tr data-dt-id="<?= $did ?>">
      ...
      <td>
        <select class="dt-move-select <?= $move_val ? 'has-value' : '' ?>"
          ...
          <?php endforeach; ?>
          <?php if ($move_val && !in_array($move_val, $registrar_options, true)): ?>
            <option value="<?= esc($move_val) ?>" selected><?= esc($move_val) ?></option>
          <?php endif; ?>
        </select>
      </td>
