...
class MOMController {
  ...
  private function createMeetingReminder($mom_id, $title, $type, $meeting_at) {
    ...
    $stmt = $this->conn->prepare("
      INSERT INTO tracs_reminders (user_id, title, description, due_date, priority, is_completed, created_at)
      VALUES (?, ?, ?, ?, ?, 0, NOW())
    ");
    if(!$stmt) return null;
    $remTitle = "MOM: " . $title;
    $stmt->bind_param('issss', $this->uid, $remTitle, $desc, $due, $priority);
    return $stmt->execute() ? (int)$stmt->insert_id : null;
  }
  ...
  public function createMOM($title, $type='weekly', $objective='', $participants='', $meeting_at=null, $meeting_url=null) {
    ...
    if($has_meeting_at && $has_meeting_url) {
      $stmt = $this->conn->prepare("
        INSERT INTO tracs_moms (title, type, objective, participants, meeting_at, meeting_url, status, created_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, 'upcoming', ?, ?, ?)
      ");
      $stmt->bind_param('ssssssiss', $title, $type, $objective, $participants, $meeting_at, $meeting_url, $this->uid, $now, $now);
    } else if($has_meeting_at) {
      $stmt = $this->conn->prepare("
        INSERT INTO tracs_moms (title, type, objective, participants, meeting_at, status, created_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 'upcoming', ?, ?, ?)
      ");
      $stmt->bind_param('sssssiss', $title, $type, $objective, $participants, $meeting_at, $this->uid, $now, $now);
    } else {
      $stmt = $this->conn->prepare("
        INSERT INTO tracs_moms (title, type, objective, participants, status, created_by, created_at, updated_at)
        VALUES (?, ?, ?, ?, 'upcoming', ?, ?, ?)
      ");
      $stmt->bind_param('ssssiss', $title, $type, $objective, $participants, $this->uid, $now, $now);
    }
    ...
    if($stmt->execute()) {
      ...
      logAct($this->conn, 'mom_scheduled', "Scheduled MOM: $title", $this->uid);
      return $mom_id;
    }
    return false;
  }
  ...
  public function getMOM($mom_id) {
    ...
    $this->ensureOperationalSchema();
    $mom_id = (int)$mom_id;
    $stmt = $this->conn->prepare("SELECT * FROM tracs_moms WHERE id=? AND created_by=? LIMIT 1");
  ...
  public function getMOMs($status='all', $limit=100) {
    ...
    $this->ensureOperationalSchema();
    $query = "SELECT * FROM tracs_moms WHERE created_by=?";
    $params = [$this->uid];
    $types = 'i';
    ...
    $order = $this->hasColumn('tracs_moms', 'meeting_at') ? 'COALESCE(meeting_at, created_at)' : 'created_at';
  ...
  public function updateMOM($mom_id, $title, $objective, $participants, $type='weekly', $status='upcoming', $meeting_at=null, $meeting_url=null) {
    ...
    if($stmt->execute()) {
      logAct($this->conn, 'mom_updated', "Updated MOM #$mom_id", $this->uid);
      return true;
    }
    return false;
  }
  ...
  public function closeMOM($mom_id) {
    ...
    if($stmt->execute()) {
      ...
      logAct($this->conn, 'mom_completed', "Completed MOM #$mom_id", $this->uid);
      return true;
    }
    return false;
  }
  ...
  public function deleteMOM($mom_id) {
    ...
    if($ok && $stmt->affected_rows > 0) {
      ...
      logAct($this->conn, 'mom_deleted', "Deleted MOM #$mom_id", $this->uid);
      return true;
    }
    return false;
  }
  ...
  public function startMOM($mom_id) {
    ...
    logAct($this->conn, 'mom_started', $msg, $this->uid);
    return true;
  }
  ...
  public function cancelMOM($mom_id) {
    ...
    logAct($this->conn, 'mom_cancelled', "Cancelled MOM #$mom_id", $this->uid);
    return true;
  }
  ...
  public function saveSummary($mom_id, $summary) {
    ...
    $ok = $stmt->execute();
    if($ok) logAct($this->conn, 'mom_summary_saved', "Saved MOM summary #$mom_id", $this->uid);
    return $ok;
  }
  ...
  public function addDecision($mom_id, $decision, $rationale='', $owner='') {
    ...
    if($stmt->execute()) {
      ...
      logAct($this->conn, 'mom_decision_recorded', "Decision recorded in MOM #$mom_id", $this->uid);
      return $id;
    }
    return false;
  }
  ...
  public function addActionItem($mom_id, $title, $description='', $assigned_to='', $priority='medium', $due_date=null) {
    ...
    if($stmt->execute()) {
      $id = $this->conn->insert_id;
      logAct($this->conn, 'mom_action_created', "Created MOM action: $title", $this->uid);
      return $id;
    }
    return false;
  }
  ...
  public function completeAction($action_id, $completed=true) {
    ...
    if($ok) {
      ...
      $rem->execute();
      logAct($this->conn, $completed ? 'action_completed' : 'action_reopened', ($completed ? 'Completed' : 'Reopened') . " action #$action_id", $this->uid);
    }
    return $ok;
  }
  ...
  public function createCaseFromAction($action_id) {
    ...
    $stmt = $this->conn->prepare("
      INSERT INTO tracs_cases (user_id, title, status, priority, next_check_at, notes, created_at, updated_at)
      VALUES (?, ?, 'active', ?, ?, ?, NOW(), NOW())
    ");
    $stmt->bind_param('issss', $this->uid, $action['title'], $action['priority'], $next, $notes);
    if(!$stmt->execute()) return false;
    ...
    logAct($this->conn, 'mom_case_created', "Created case from MOM action #$action_id", $this->uid);
  ...
  public function createReminderFromAction($action_id) {
    ...
    $stmt = $this->conn->prepare("
      ...
      VALUES (?, ?, ?, ?, ?, 0, NOW())
    ");
    $stmt->bind_param('issss', $this->uid, $action['title'], $desc, $due, $action['priority']);
    if(!$stmt->execute()) return false;
    ...
    logAct($this->conn, 'action_reminder_created', "Created reminder for action #$action_id", $this->uid);
  ...
  public function resolveLinkedCaseFromMOM($mom_id, $case_id, $status='completed', $note='') {
    ...
    logAct($this->conn, 'mom_case_status_updated', "Case #$case_id marked as $label after MOM review", $this->uid);
  ...
  public function attachScreenshot($mom_id, $image_data, $attached_to_type='general', $attached_to_id=null) {
    ...
    if($bytes && strlen($bytes) <= 5 * 1024 * 1024 && file_put_contents($filepath, $bytes)) {
      ...
      if($stmt->execute()) {
        logAct($this->conn, 'mom_screenshot_uploaded', "Uploaded screenshot for MOM #$mom_id", $this->uid);
  ...
  public function deleteScreenshot($screenshot_id) {
    ...
    logAct($this->conn, 'mom_screenshot_deleted', "Deleted MOM screenshot #$screenshot_id", $this->uid);
    return true;
  }
}
