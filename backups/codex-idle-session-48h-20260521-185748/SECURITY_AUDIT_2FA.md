...
8. Full sessions also expire when the 2FA verification age reaches `TRACS_2FA_REMEMBER_HOURS`, capped at 48 hours. TV mode shows the remaining secure-session time and sends the user back to login after expiry.
...
- Protected pages, APIs, and exports enforce the absolute 2FA session age cap, so polling surfaces such as TV mode cannot keep a 2FA-authenticated session alive beyond 48 hours.
