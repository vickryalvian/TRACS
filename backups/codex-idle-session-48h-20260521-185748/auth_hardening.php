...
const TRACS_AUTH_SESSION_EXPIRED = 'Your secure session expired. Please sign in again.';
...
function tracs_two_factor_remember_seconds(): int {
    return tracs_auth_config_int('TRACS_2FA_REMEMBER_HOURS', 48, 1, 48) * 3600;
}
...
function tracs_two_factor_schema_ready(mysqli $conn): bool {
    return function_exists('tracs_column_exists')
        ...
        && tracs_column_exists($conn, 'tracs_users', 'two_factor_last_verified_at');
}

function tracs_two_factor_index_exists(mysqli $conn, string $index): bool {
    $stmt = $conn->prepare("
        SELECT 1
...
function tracs_two_factor_ensure_schema(mysqli $conn): bool {
    if (tracs_two_factor_schema_ready($conn)) {
        return true;
    }
    if (!function_exists('tracs_table_exists') || !function_exists('tracs_column_exists') || !tracs_table_exists($conn, 'tracs_users')) {
        return false;
    }
    ...
    try {
        foreach ($columns as $column => $definition) {
            if (!tracs_column_exists($conn, 'tracs_users', $column)) {
                $conn->query("ALTER TABLE `tracs_users` ADD COLUMN `{$column}` {$definition}");
            }
        }
        $conn->query("
            UPDATE `tracs_users`
        ...
        ");
        if (!tracs_two_factor_index_exists($conn, 'idx_tracs_users_2fa_required')) {
            $conn->query('ALTER TABLE `tracs_users` ADD INDEX `idx_tracs_users_2fa_required` (`two_factor_reset_required`, `two_factor_enabled`)');
        }
        if (!tracs_two_factor_index_exists($conn, 'idx_tracs_users_2fa_lock')) {
            $conn->query('ALTER TABLE `tracs_users` ADD INDEX `idx_tracs_users_2fa_lock` (`two_factor_locked_until`)');
        }
    } catch (Throwable $e) {
        error_log('TRACS 2FA schema bootstrap failed: ' . $e->getMessage());
        return false;
    }

    return tracs_two_factor_schema_ready($conn);
}
...
function tracs_auth_two_factor_verified_at(): int {
    tracs_start_session();
    return (int)($_SESSION['tracs_2fa_verified_at'] ?? 0);
}

function tracs_auth_two_factor_expires_at(): int {
    $verifiedAt = tracs_auth_two_factor_verified_at();
    if ($verifiedAt <= 0) {
        return 0;
    }
    return $verifiedAt + tracs_two_factor_remember_seconds();
}

function tracs_auth_two_factor_remaining_seconds(): int {
    $expiresAt = tracs_auth_two_factor_expires_at();
    if ($expiresAt <= 0) {
        return 0;
    }
...
function tracs_auth_two_factor_session_expired(): bool {
    return tracs_auth_two_factor_remaining_seconds() <= 0;
}

function tracs_auth_destroy_current_session(): void {
    $_SESSION = [];
