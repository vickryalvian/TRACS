...
if (tracs_auth_two_factor_session_expired()) {
    $expiredUserId = (int)($_SESSION['user_id'] ?? 0);
    tracs_auth_destroy_current_session();
    $_SESSION['login_error'] = TRACS_AUTH_SESSION_EXPIRED;
    $_SESSION['login_show_help'] = true;
    if (isset($conn) && $conn instanceof mysqli) {
        tracs_auth_log_event($conn, 'session_timeout', 'expired', '', $expiredUserId ?: null, 'two_factor_max_age');
    }
    header('Location: /login.php');
    exit;
}
