...
- Configure 2FA as needed with `TRACS_2FA_ISSUER`, `TRACS_2FA_TIMEOUT_MINUTES`, `TRACS_2FA_REMEMBER_HOURS`, `TRACS_2FA_MAX_FAILED_ATTEMPTS`, `TRACS_2FA_LOCK_MINUTES`, `TRACS_2FA_VALID_WINDOW_STEPS`, and `TRACS_2FA_SECRET_KEY`. `TRACS_2FA_REMEMBER_HOURS` is capped at 48 hours, including TV mode sessions. Production deployments should set `TRACS_2FA_SECRET_KEY` to a long random value and keep it stable across deploys.
...
- Fully authenticated sessions have an absolute 2FA age limit controlled by `TRACS_2FA_REMEMBER_HOURS`, capped at 48 hours. TV mode displays the remaining secure-session time and redirects to login when the cap expires.
