...
(function () {
  ...
  function setSessionCountdown() {
    const label = $('[data-tv-session-remaining]');
    if (!label) return;
    const card = label.closest('.tv-session-card');
    const info = window.TRACS_SESSION_INFO || {};
    const expiresAtMs = Number(info.twoFactorExpiresAt || 0) * 1000;
    const remaining = expiresAtMs > 0 ? Math.floor((expiresAtMs - Date.now()) / 1000) : 0;
    label.textContent = remaining > 0 ? formatSessionTime(remaining) : 'Expired';
    card?.classList.toggle('is-warning', remaining > 0 && remaining <= 30 * 60);
  ...
  setSessionCountdown();
  setInterval(setSessionCountdown, 1000);
  setInterval(() => {
