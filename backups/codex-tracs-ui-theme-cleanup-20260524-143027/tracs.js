...
async function openEditCase(id){
  ...
  if(d.success){
    const data=d.data||{};
    setVal('caseTitle',data.title||row.dataset.title||'');
    setVal('caseStatus',data.status||row.dataset.status||'active');
    setVal('casePriority',data.priority||row.dataset.priority||'medium');
    setVal('caseNextCheck',data.next_check_at?data.next_check_at.replace(' ','T').slice(0,16):(row.dataset.next||''));
...
async function saveCase(){
  ...
  const d=useForm
    ? await caseApiWithUploads(id?API.CASE.UPDATE:API.CASE.CREATE,casePayloadFormData(id))
    : await api(id?API.CASE.UPDATE:API.CASE.CREATE,{id,title,status:val('caseStatus'),priority:val('casePriority'),next_check_at:val('caseNextCheck'),notes:val('caseNotes')});
  if(saveBtn){saveBtn.disabled=false;saveBtn.classList.remove('is-loading');}
  if(d.success){toast(id?'Case updated':'Case created','success');closeModal('case');clearCaseAttachmentState();_reload();}
  else {
    caseSetUploadStatus(d.message||'Error saving case','error');
    toast(d.message||'Error saving case','error');
  }
}
...
const TRACS_VISUAL_THEME_CHOICES = new Set(['default', 'tracs_v2']);
...
function tracsNormalizeVisualThemePreference(value) {
  ...
  if (normalized === 'tracsv2' || normalized === 'intercom_inspired') return 'tracs_v2';
  return TRACS_VISUAL_THEME_CHOICES.has(normalized) ? normalized : '';
}

function tracsVisualThemeAttribute(value) {
  return tracsNormalizeVisualThemePreference(value) === 'tracs_v2' ? 'tracs-v2' : 'default';
}
