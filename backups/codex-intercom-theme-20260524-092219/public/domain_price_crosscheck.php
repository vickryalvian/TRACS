            ...
            <?php foreach ($active_sources as $source): ?>
              <?php if (!dpc_is_internal_cost_source($source)): ?>
                ...
                <tr class="row-source-idr">
                  ...
                  ?>
                    <td class="auto-calc cell-idr" data-live-idr data-tld="<?=$tld['id']?>" data-source="<?=$source['id']?>" data-type="cost_register">
                ...
                <tr class="row-source-idr">
                  ...
                  <?php foreach ($active_tlds as $tld):
                    ...
                    $idr = $val ? number_format($val['idr_value'], 0, '', '') : '';
                  ?>
                    <td class="auto-calc cell-idr" data-live-idr data-tld="<?=$tld['id']?>" data-source="<?=$source['id']?>" data-type="cost_renewal">
                      <?= $idr ? 'Rp ' . number_format($idr, 0, ',', '.') : '<span class="dpc-empty-value">—</span>' ?>
                    </td>
                  <?php endforeach; ?>
                </tr>
          ...
          <div class="dpc-extension-rows">
            <?php foreach ($active_tlds as $tld): ?>
              <form method="post" class="dpc-extension-edit-row" data-extension-edit-form>
                ...
                <span class="dpc-extension-name" data-extension-name="<?=esc(strtolower((string)$tld['tld_name']))?>"><?=esc($tld['tld_name'])?></span>
                <span class="dpc-extension-category-badge">gTLD</span>
                <input type="number" name="sort_order" class="form-input" min="1" max="999999" step="1" value="<?=esc((string)$tld['sort_order'])?>" aria-label="Display order for <?=esc($tld['tld_name'])?>" title="Display Order: lower numbers appear first" <?=$month_data && !$is_intern && tracs_user_can($conn, 'domain_price.manage') ? '' : 'disabled'?>>
                <?php if ($month_data && !$is_intern && tracs_user_can($conn, 'domain_price.manage')): ?>
                  <div class="dpc-extension-row-actions">
                    ...
                    <?php if ($is_super_admin): ?>
                      <button type="submit" name="action" value="delete_tld_extension" class="btn btn-danger btn-sm" data-delete-extension="<?=esc($tld['tld_name'])?>" title="Delete extension"><i data-lucide="trash-2" class="icon-xs"></i></button>
                    <?php endif; ?>
                  </div>
                <?php endif; ?>
              </form>
          ...
          <div class="dpc-extension-rows">
            <?php foreach ($active_cctlds as $tld): ?>
              <form method="post" class="dpc-extension-edit-row" data-extension-edit-form>
                ...
                <span class="dpc-extension-name" data-extension-name="<?=esc(strtolower((string)$tld['tld_name']))?>"><?=esc(strtoupper($tld['tld_name']))?></span>
