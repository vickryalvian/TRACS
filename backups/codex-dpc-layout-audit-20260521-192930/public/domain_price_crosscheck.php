...
if (!tracs_user_can($conn, 'domain_price.view')) {
    ...
    echo "<html><head><title>403 Forbidden</title></head><body style='font-family:sans-serif;background:#0d1117;color:#c9d1d9;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;'>";
    echo "<h1 style='color:#f85149;font-size:3rem;margin:0 0 10px 0;'>403</h1>";
    echo "<h2 style='margin:0 0 20px 0;font-weight:500;'>Access Denied</h2>";
    echo "<p style='color:#8b949e;margin:0 0 30px 0;max-width:400px;text-align:center;'>You do not have the required permissions (domain_price.view) to view the Domain Price Crosscheck panel.</p>";
    echo "<a href='index.php' style='color:#58a6ff;text-decoration:none;border:1px solid #30363d;padding:8px 16px;border-radius:6px;background:#21262d;'>Return to Dashboard</a>";
    echo "</body></html>";
    exit();
}
...
<div class="main-inner">
  ...
  <div class="tracs-alert alert-danger" style="margin-bottom:20px;">
    <i data-lucide="alert-circle" class="icon-sm"></i>
    <span><?=esc($error_message)?></span>
  </div>
  <?php endif; ?>
  <?php if (!empty($success_message)): ?>
  <div class="tracs-alert alert-success" style="margin-bottom:20px;">
  ...
  <?php else: ?>
    ...
    <div class="panel dpc-matrix-panel" id="gtld-pricing">
      <div class="panel-head">
        ...
        <div class="panel-right" style="display: flex; gap: 8px;">
      ...
      <div class="table-container" style="overflow-x: auto; max-width: 100%;">
        <table class="table-dense dpc-matrix-table" style="width: 100%; min-width: 800px; border-collapse: separate; border-spacing: 0;">
          <thead>
            <tr>
              <th class="sticky-col group-header-col" style="z-index: 3; left: 0;">Source Group</th>
              <th class="sticky-col type-col" style="z-index: 3; left: 160px; border-right: 2px solid var(--bd2);">Type</th>
              <?php foreach ($active_tlds as $tld): ?>
                <th class="tld-col" style="min-width: 140px;"><?=esc($tld['tld_name'])?></th>
              <?php endforeach; ?>
            </tr>
          </thead>
          <tbody>
            ...
            <?php foreach ($active_sources as $source): ?>
              <?php if (!dpc_is_internal_cost_source($source)): ?>
                ...
                <tr class="row-source-usd">
                  <td class="sticky-col group-header-col" rowspan="4" style="left: 0; background: var(--s1); vertical-align: top; padding-top: 12px;">
                    <strong><?=esc($source['source_name'])?></strong>
                  </td>
                  <td class="sticky-col type-col" style="left: 160px; background: var(--s1); border-right: 2px solid var(--bd2);">Register (USD)</td>
                  <?php foreach ($active_tlds as $tld):
                ...
                <tr class="row-source-idr">
                  <td class="sticky-col type-col auto-calc-col" style="left: 160px; background: var(--s2); border-right: 2px solid var(--bd2);">
                    <span style="font-size: 11px; color: var(--tx3);">Register (IDR)</span>
                  </td>
                  <?php foreach ($active_tlds as $tld):
                    $val = $matrix_data[$tld['id']][$source['id']]['cost_register'] ?? null;
                    $idr = $val ? number_format($val['idr_value'], 0, '', '') : '';
                  ?>
                    <td class="auto-calc cell-idr" style="background: var(--s2); text-align: right; padding-right: 12px; font-family: var(--mono); font-size: 13px; color: var(--tx2);">
                      <?= $idr ? 'Rp ' . number_format($idr, 0, ',', '.') : '<span style="color:var(--tx3)">—</span>' ?>
                    </td>
                  <?php endforeach; ?>
                </tr>
                <!-- Renewal Cost (USD input) -->
                <tr class="row-source-usd">
                  <td class="sticky-col type-col" style="left: 160px; background: var(--s1); border-right: 2px solid var(--bd2);">Renewal (USD)</td>
                  <?php foreach ($active_tlds as $tld):
                ...
                <tr class="row-source-idr">
                  <td class="sticky-col type-col auto-calc-col" style="left: 160px; background: var(--s2); border-right: 2px solid var(--bd2);">
                    <span style="font-size: 11px; color: var(--tx3);">Renewal (IDR)</span>
                  </td>
                  <?php foreach ($active_tlds as $tld):
                    $val = $matrix_data[$tld['id']][$source['id']]['cost_renewal'] ?? null;
                    $idr = $val ? number_format($val['idr_value'], 0, '', '') : '';
            ...
            <tr class="matrix-divider"><td colspan="<?=count($active_tlds) + 2?>" style="height: 4px; background: var(--bd1); padding: 0;"></td></tr>

            <!-- IDCH Internal cost rows -->
            <?php foreach ($active_sources as $source): ?>
              <?php if (dpc_is_internal_cost_source($source)): ?>
                <tr class="row-internal-idr">
                  <td class="sticky-col group-header-col" rowspan="2" style="left: 0; background: var(--s1); vertical-align: top; padding-top: 12px;">
                    <strong><?=esc($source['source_name'])?></strong>
                  </td>
                  <td class="sticky-col type-col" style="left: 160px; background: var(--s1); border-right: 2px solid var(--bd2);">Register (IDR)</td>
                  <?php foreach ($active_tlds as $tld):
                    $val = $matrix_data[$tld['id']][$source['id']]['cost_register'] ?? null;
                    $idr = $val ? number_format($val['idr_value'], 0, '', '') : '';
                  ?>
                ...
                <tr class="row-internal-idr">
                  <td class="sticky-col type-col" style="left: 160px; background: var(--s1); border-right: 2px solid var(--bd2);">Renewal (IDR)</td>
                  <?php foreach ($active_tlds as $tld):
                    $val = $matrix_data[$tld['id']][$source['id']]['cost_renewal'] ?? null;
                    $idr = $val ? number_format($val['idr_value'], 0, '', '') : '';
                  ?>
            ...
            <table class="table-dense" style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr style="border-bottom: 1px solid var(--color-border); text-align: left;">
                  <th style="padding: 8px;">TLD</th>
                  <th style="padding: 8px;">Follow-up Status</th>
              ...
              <tbody>
                <?php foreach ($tld_notes as $note): ?>
                  <tr style="border-bottom: 1px solid var(--color-border);">
                    <td style="padding: 8px;"><strong><?=$note['tld_name']?></strong></td>
                    ...
                    <td style="padding: 8px;"><?=esc($note['manual_note'])?></td>
                    <td style="padding: 8px; color: var(--tx3); font-size: 0.85em;">
    ...
    <div class="dpc-lower-layout" id="record-operations">
      ...
      <section class="panel dpc-meta-panel">
        ...
        <div class="panel-head" style="margin-top: 20px; border-top: 1px solid var(--color-border); padding-top: 15px;">
          <span class="panel-title">
        ...
        <?php else: ?>
          <div class="empty-small" style="margin-bottom: 15px;">No task assigned for this month.</div>
          <?php if (!$is_intern && tracs_user_can($conn, 'domain_price.manage')): ?>
            <button class="btn btn-outline" style="width: 100%;" onclick="openAssignTaskModal(<?=$month_data['id']?>)">
