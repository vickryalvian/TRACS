...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create_month' && tracs_user_can($conn, 'domain_price.manage')) {
            ...
            header("Location: domain_price_crosscheck.php" . ($target_month > 0 ? "?month_id={$target_month}" : "") . "#extension-manager");
            exit();
...
<div class="main-inner">
  ...
  <?php else: ?>
    ...
    <div class="dpc-section-tabs" aria-label="Domain price sections">
      ...
      <a href="#extension-manager">Extensions</a>
      <a href="#tld-notes">Notes</a>
      <a href="#record-operations">Audit / Metadata</a>
    </div>

    <section class="panel dpc-extension-panel" id="extension-manager">
      <div class="panel-head">
        <span class="panel-title">
          <i data-lucide="globe-2" class="icon-sm"></i>
          Domain Extensions
        </span>
        <span class="panel-meta"><?=count($active_tlds)?> gTLD · <?=count($active_cctlds)?> ccTLD active</span>
      </div>
      <div class="dpc-extension-body">
        <?php if (!$is_intern && tracs_user_can($conn, 'domain_price.manage')): ?>
        <!-- Layout audit: extension creation lives in-page so new columns appear in the real pricing matrices after refresh. -->
        <form method="post" class="dpc-extension-form">
          <?= csrf_input() ?>
          <input type="hidden" name="action" value="add_tld_extension">
          <input type="hidden" name="month_id" value="<?= (int)$month_data['id'] ?>">
          <div class="form-group">
            <label>Extension</label>
            <input type="text" name="tld_name" class="form-input" placeholder=".app or .web.id" required>
          </div>
          <div class="form-group">
            <label>Category</label>
            <select name="tld_category" class="form-select" required>
          ...
          <div class="form-group">
            ...
            <input type="number" name="sort_order" class="form-input" min="1" placeholder="Optional">
          </div>
          <button type="submit" class="btn btn-primary">
            <i data-lucide="plus-circle" class="icon-sm"></i>
            Add Extension
          ...
          <div>
            ...
            <div class="dpc-extension-chips">
              <?php foreach ($active_cctlds as $tld): ?>
                <span><?=esc(strtoupper($tld['tld_name']))?></span>
              <?php endforeach; ?>
            </div>
          </div>
        </div>
