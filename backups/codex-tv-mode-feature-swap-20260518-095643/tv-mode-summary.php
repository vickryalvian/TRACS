...
try {
    ...
    $metrics = [
        ...
        'active_moms' => tv_table_exists($conn, 'tracs_moms') ? tv_scalar($conn, "SELECT COUNT(*) n FROM tracs_moms WHERE status IN ('upcoming','ongoing') AND (meeting_at IS NULL OR meeting_at >= DATE_SUB(NOW(), INTERVAL 2 HOUR))") : 0,
    ...
    $momRows = tv_table_exists($conn, 'tracs_moms') ? tv_rows($conn, "
        SELECT title, meeting_at, meeting_url, status, type
        FROM tracs_moms
        WHERE status IN ('upcoming','ongoing')
        ORDER BY CASE WHEN status='ongoing' THEN 1 ELSE 2 END, COALESCE(meeting_at, created_at) ASC
        LIMIT 3
    ") : [];
    $meetings = array_map(fn($r) => [
        'title' => tv_clean_title($r['title'] ?? '', 60),
        'time' => !empty($r['meeting_at']) ? date('H:i', strtotime($r['meeting_at'])) : 'TBD',
        'due' => tv_due($r['meeting_at'] ?? null),
        'status' => (string)($r['status'] ?? 'upcoming'),
        'type' => ucfirst((string)($r['type'] ?? 'meeting')),
        'has_url' => !empty($r['meeting_url']),
        'soon' => !empty($r['meeting_at']) && strtotime($r['meeting_at']) >= time() && strtotime($r['meeting_at']) <= strtotime('+30 minutes'),
    ], $momRows);
    ...
    $domainAlerts = tv_table_exists($conn, 'tracs_domains') ? tv_scalar($conn, "SELECT COUNT(*) n FROM tracs_domains WHERE expires_at IS NOT NULL AND expires_at <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)") : 0;
    $financeAlerts = tv_table_exists($conn, 'tracs_finance_transfers') ? tv_scalar($conn, "SELECT COUNT(*) n FROM tracs_finance_transfers WHERE status IN ('pending','failed') AND transfer_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)") : 0;
    ...
    } elseif ($meetings && ($meetings[0]['soon'] ?? false)) {
        $spotlight = ['type' => 'Meeting Soon', 'severity' => 'watch', 'title' => $meetings[0]['title'], 'detail' => 'Meeting starts soon. Link is available on the MoM page.', 'meta' => $meetings[0]['due']];
    }
    ...
    foreach ($meetings as $m) if ($m['soon'] || $m['status'] === 'ongoing') $ticker[] = ['tone' => 'info', 'text' => ($m['status'] === 'ongoing' ? 'Live meeting: ' : 'Meeting soon: ') . $m['title']];
