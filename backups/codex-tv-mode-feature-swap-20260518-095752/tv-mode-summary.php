...
function tv_role_allowed(array $user): bool {
    return in_array((string)($user['role_slug'] ?? ''), ['super_admin', 'admin', 'supervisor'], true);
}
...
if (!tv_role_allowed($authUser ?? [])) fail('Forbidden', 403);
...
try {
    ...
    $opsWatch = [];
    if (tv_table_exists($conn, 'domain_transfers')) {
        foreach (tv_rows($conn, "
            SELECT domain_name, transfer_status, process_start_date, updated_at
            FROM domain_transfers
            WHERE transfer_status IN ('pending transfer','locked','error epp code','pending verification','renew period')
            ORDER BY FIELD(transfer_status,'error epp code','locked','pending verification','pending transfer','renew period'), COALESCE(process_start_date, updated_at) ASC
            LIMIT 3
        ") as $r) {
            $tone = in_array((string)$r['transfer_status'], ['error epp code','locked'], true) ? 'critical' : 'watch';
            $opsWatch[] = [
                'title' => tv_clean_title($r['domain_name'] ?? 'Domain transfer', 54),
                'meta' => 'Domain / ' . tv_clean_title($r['transfer_status'] ?? 'pending', 28) . ' / ' . tv_age($r['process_start_date'] ?? $r['updated_at'] ?? null),
                'tone' => $tone,
            ];
        }
    }
    if (tv_table_exists($conn, 'tracs_domains')) {
        foreach (tv_rows($conn, "
            SELECT domain, expires_at, auto_renew, updated_at
            FROM tracs_domains
            WHERE expires_at IS NOT NULL AND expires_at <= DATE_ADD(CURDATE(), INTERVAL 7 DAY)
            ORDER BY expires_at ASC
            LIMIT 3
        ") as $r) {
            $opsWatch[] = [
                'title' => tv_clean_title($r['domain'] ?? 'Domain expiry', 54),
                'meta' => 'Domain expiry / ' . tv_due($r['expires_at'] ?? null) . (!empty($r['auto_renew']) ? ' / auto-renew' : ''),
                'tone' => !empty($r['expires_at']) && strtotime((string)$r['expires_at']) <= strtotime('+2 days') ? 'critical' : 'watch',
            ];
        }
    }
    if (tv_table_exists($conn, 'balance_transfers')) {
        foreach (tv_rows($conn, "
            SELECT amount, status, transfer_date, ticket_id
        ...
        ") as $r) {
            $opsWatch[] = [
                'title' => 'Pending balance transfer',
                'meta' => 'Finance / ' . tv_age($r['transfer_date'] ?? null) . ' / ticket ' . tv_clean_title($r['ticket_id'] ?? 'N/A', 18),
                'tone' => 'watch',
            ];
        }
    }
    if (tv_table_exists($conn, 'tracs_finance_transfers')) {
        foreach (tv_rows($conn, "
            SELECT note, status, transfer_date
            FROM tracs_finance_transfers
            WHERE status IN ('pending','failed') AND transfer_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)
            ORDER BY CASE WHEN status='failed' THEN 1 ELSE 2 END, transfer_date ASC
            LIMIT 3
        ") as $r) {
            $opsWatch[] = [
                'title' => tv_clean_title($r['note'] ?? 'Finance movement', 54),
                'meta' => 'Finance / ' . tv_clean_title($r['status'] ?? 'pending', 18) . ' / ' . tv_age($r['transfer_date'] ?? null),
                'tone' => ($r['status'] ?? '') === 'failed' ? 'critical' : 'watch',
            ];
        }
    }
    if (tv_table_exists($conn, 'tracs_cancellation_feedback')) {
        foreach (tv_rows($conn, "
            SELECT cancelled_service, cancellation_reason, created_at
        ...
        ") as $r) {
            $opsWatch[] = [
                'title' => tv_clean_title($r['cancelled_service'] ?? 'Cancellation feedback', 54),
                'meta' => 'Feedback / ' . tv_clean_title($r['cancellation_reason'] ?? 'recent', 36) . ' / ' . tv_age($r['created_at'] ?? null),
                'tone' => 'info',
            ];
        }
    }
    $opsWatch = array_slice($opsWatch, 0, 5);
    ...
    $activityRows = tv_table_exists($conn, 'tracs_activity_logs') ? tv_rows($conn, "
        ...
        WHERE module IN ('case','cases','reminder','reminders','checklist','task','mom','moms','shift','shift-reports','finance','domains','cancellation-feedback')
        ORDER BY created_at DESC
        LIMIT 8
    ") : [];
    ...
    if ($cases && (($cases[0]['priority'] ?? '') === 'critical' || ($cases[0]['attention'] ?? false))) {
        ...
        $spotlight = ['type' => 'Ops Watch', 'severity' => $opsWatch[0]['tone'] === 'critical' ? 'critical' : 'watch', 'title' => $opsWatch[0]['title'], 'detail' => 'Finance, domain, or cancellation signal needs operational awareness.', 'meta' => $opsWatch[0]['meta']];
    }
