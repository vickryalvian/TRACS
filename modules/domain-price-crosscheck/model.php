...
class DomainPriceCrosscheckModel {
    ...
    /**
     * Get a TLD extension by normalized name.
     */
    public function getTldByName(string $tldName): ?array {
        $stmt = $this->db->prepare("SELECT id, tld_name, tld_category, is_active, sort_order FROM domain_price_tlds WHERE tld_name = ? LIMIT 1");
        if (!$stmt) return null;
        $stmt->bind_param("s", $tldName);
        $stmt->execute();
    ...
    /**
     * Create or reactivate a TLD extension used by the pricing matrices.
     */
    public function saveTldExtension(string $tldName, string $category, int $sortOrder): ?int {
        $sql = "INSERT INTO domain_price_tlds (tld_name, tld_category, is_active, sort_order, created_at)
                VALUES (?, ?, 1, ?, NOW())
        ...
        $stmt = $this->db->prepare($sql);
        if (!$stmt) return null;
        $stmt->bind_param("ssi", $tldName, $category, $sortOrder);
        ...
        $lookup = $this->db->prepare("SELECT id FROM domain_price_tlds WHERE tld_name = ? LIMIT 1");
        if (!$lookup) return null;
        $lookup->bind_param("s", $tldName);
        $lookup->execute();
