  ...
  return (
    <div className="tr:grid tr:grid-cols-1 tr:gap-tracs-2 tr:md:hidden">
      {assignments.map((assignment) => (
        <article
          className="tr:rounded-tracs tr:border tr:border-tracs-border tr:bg-tracs-surface-2 tr:p-tracs-3"
          key={assignment.id}
        >
          <div className="tr:flex tr:items-start tr:justify-between tr:gap-tracs-2">
            <div>
              <strong className="tr:block tr:text-sm tr:text-tracs-primary">
          ...
          <div className="tr:mt-tracs-3 tr:grid tr:grid-cols-2 tr:gap-tracs-2 tr:text-xs">
            ...
            <span className="tr:text-right tr:text-tracs-muted">{assignment.division.name}</span>
          </div>
        </article>
      ))}
    </div>
