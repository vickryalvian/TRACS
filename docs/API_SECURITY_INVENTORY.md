...
| Future Shift Assignment v1 copy contracts | Planned `templates/copy-preview.php` and `templates/copy-commit.php` | POST | Future granular template permissions plus object/division scope; exact `super_admin` plus `shifts.manage` until seeded | Required | Critical; Phase 38 contract-only gate documents non-mutating copy-preview, source/target validation, CSRF, conflict/warning/blocked item behavior, and future `APPLY COPY`; no copy public route exists |
