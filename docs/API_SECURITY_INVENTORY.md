...
| Future Shift Assignment v1 template commit/copy contracts | Planned `templates/commit.php`, `templates/copy-preview.php`, and `templates/copy-commit.php` | POST | Future granular template permissions plus object/division scope; exact `super_admin` plus `shifts.manage` until seeded | Required | Critical; no commit/copy public route exists; Phase 30 requires preview-to-commit integrity, exact `APPLY TEMPLATE`, final conflict re-check, audit, and bulk rollback evidence before implementation |
