...
| `api/v1/shift-assignment/templates/copy-preview.php` | Non-mutating copy schedule preview | POST | Exact `super_admin` plus explicit `shifts.manage`; also requires `shifts.template.copy_preview` if seeded later | Required | High; Phase 39 side-effect-free source-to-target preview only, target conflict/warning/blocked item output; Phase 40 React UI calls this route with CSRF but still has no copy-commit/apply behavior |
