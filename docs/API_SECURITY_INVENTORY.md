...
| `api/v1/shift-assignment/templates/preview.php` | Non-mutating template generation preview | POST | Exact `super_admin` plus explicit `shifts.manage` until granular template permission migration | Required | High; Phase 28 side-effect-free preview only, disposable no-mutation validation passed |
