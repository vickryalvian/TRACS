...
| `api/v1/shift-assignment/assignment.php?id=<id>` | Controlled scoped assignment update | PATCH | Exact `super_admin` plus explicit role assignment for `shifts.manage` until `shifts.update` migration | Required | Critical; Phase 18 disposable integration passed, no React caller |
| Future Shift Assignment v1 mutations | Planned template/overtime/warning contracts; DELETE blocked | POST/PATCH | Exact operation permission plus object/division scope | Required | Critical; no additional public route exists |
