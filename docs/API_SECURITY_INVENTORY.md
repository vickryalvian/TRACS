...
| `api/v1/shift-assignment/assignment.php?id=<id>` | Controlled scoped assignment update/delete | PATCH / DELETE | Exact `super_admin` plus explicit role assignment for `shifts.manage` until operation-specific permission migration | Required | Critical; direct-URL React pilot uses byte-exact typed confirmation; backend is fail-closed, snapshots warnings/holiday coverage, blocks template-item links, and passed Phase 26 combined regression/restoration validation |
| `api/v1/shift-assignment/templates/preview.php` | Non-mutating template generation preview | POST | Exact `super_admin` plus explicit `shifts.manage` until granular template permission migration | Required | High; Phase 28 side-effect-free preview only, disposable no-mutation validation passed; Phase 29 React caller is preview-only and exposes no commit/copy control |
| `api/v1/shift-assignment/templates/commit.php` | Controlled template commit | POST | Exact `super_admin` plus explicit `shifts.manage`; also requires `shifts.template.commit` if seeded later | Required | Critical; Phase 32 backend-only bulk write, exact `APPLY TEMPLATE`, preview recomputation, conflict `409`, created-ID rollback targeting; Phase 33 adds rollback and race-conflict disposable drills; Phase 35 React caller is gated to successful preview plus exact confirmation; Phase 37 live Chrome validation passed through the guarded test session path against `tracs_phase37_test` |
| `api/v1/shift-assignment/templates/copy-preview.php` | Non-mutating copy schedule preview | POST | Exact `super_admin` plus explicit `shifts.manage`; also requires `shifts.template.copy_preview` if seeded later | Required | High; Phase 39 side-effect-free source-to-target preview only, target conflict/warning/blocked item output; Phase 40/41 React UI calls this route with CSRF, accessible validation, stale-preview handling, browser no-mutation proof, and still no copy-commit/apply behavior |
| Future Shift Assignment v1 copy commit | Planned `templates/copy-commit.php` | POST | Future granular template permissions plus object/division scope; exact `super_admin` plus `shifts.manage` until seeded | Required | Critical; Phase 42 contract gate requires exact `APPLY COPY`, server-side preview recomputation, final conflict re-check, atomic all-or-nothing writes, created-ID audit, rollback targeting, and fail-closed behavior; Phase 43 environment gate requires disposable DB, browser validation, cleanup, and copy-commit absence preflight before implementation; no copy-commit public route exists |
| Future Shift Assignment v1 mutations | Planned overtime/warning/export/bulk contracts | POST/PATCH | Exact operation permission plus object/division scope | Required where mutating | Critical; no additional public route exists |
