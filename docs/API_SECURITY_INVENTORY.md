...
| Future Shift Assignment v1 copy commit | Planned `templates/copy-commit.php` | POST | Future granular template permissions plus object/division scope; exact `super_admin` plus `shifts.manage` until seeded | Required | Critical; future exact `APPLY COPY`, server-side revalidation, created-ID audit, and rollback targeting required; no copy-commit public route exists |
