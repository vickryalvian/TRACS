...
| Future Shift Assignment v1 template contracts | Planned `templates/preview.php`, `templates/commit.php`, `templates/copy-preview.php`, and `templates/copy-commit.php` | POST | Future granular template permissions plus object/division scope; exact `super_admin` plus `shifts.manage` until seeded | Required | Critical; Phase 27 contract only, preview routes must be non-mutating, no public route exists |
